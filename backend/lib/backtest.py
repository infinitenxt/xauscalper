...
Reuses the exact same ``strategy.analyze`` / ``plan_levels`` code the live engine
uses, then simulates the same management stack (break-even, partial TP, trailing
stop, hard time cap) bar by bar using each candle's high/low. CPU-bound, so the
router runs it via ``asyncio.to_thread``.

Honest-by-design: entries fill at the signal bar's close, and when a bar's range
covers both stop and target the STOP is taken first. Results are therefore
conservative, and are still a simulation — not a promise of live behaviour.
"""
...
def run(
    ...
    while i < n - 2:
        ...
        for j in range(i + 1, min(i + 1 + max_bars, n)):
            bar = candles[j]
            bars_held = j - i
            # Stop first when a single bar spans both levels (conservative).
            if (long and bar["low"] <= sl) or (not long and bar["high"] >= sl):
                exit_price = sl
                exit_reason = (
                    "TRAILING STOP" if trailing else ("BREAK-EVEN STOP" if breakeven_done else "STOP LOSS")
                )
                break
            ...
            if r_mult >= trail_start:
                cand = close - trail_mult * atr if long else close + trail_mult * atr
                floor_ = entry + 0.3 * r if long else entry - 0.3 * r
                new_sl = max(cand, floor_) if long else min(cand, floor_)
    ...
    if total == 0:
        out = _empty(
            "No setup in this window cleared every gate — with an 80% threshold that is normal "
            "for a quiet stretch of gold."
        )
    ...
    return {
        ...
        "note": (
            "Simulation on real Binance gold candles using the live rules. Entries fill at the "
            "signal bar's close and a bar covering both levels is scored as a stop, so results "
            "lean pessimistic. Past behaviour is not a prediction."
        ),
    }
