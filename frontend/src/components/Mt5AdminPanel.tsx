...
export default function Mt5AdminPanel() {
  ...
  const patchPlan = useMutation({ mutationFn: (body: Partial<Plan>) => apiPatch<Plan>(`/admin/plans/${plan?.id}`, body), onSuccess: () => { toast.success("MT5 Auto-Trading plan updated"); void qc.invalidateQueries({ queryKey: ["admin", "plans"] }); }, onError: (err) => toast.error(errorText(err)) });
